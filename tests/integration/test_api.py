...
class APITester:
    ...
    def test_health_endpoint(self):
        ...
        try:
            ...
            assert (
                data["status"] == "healthy"
            ), f"Status não é 'healthy': {data['status']}"
    ...
    def test_prediction_valid_ticker(self, ticker: str):
        ...
        try:
            ...
            ), f"Ticker esperado {ticker}, recebido {result['ticker']}"
            assert isinstance(
                result["predicted_price"], (int, float)
            ), "predicted_price não é número"
            assert isinstance(
                result["current_price"], (int, float)
            ), "current_price não é número"
            assert result["confidence"] in [
                "high",
                "medium",
                "low",
            ], f"Confidence inválido: {result['confidence']}"
    ...
    def test_prediction_invalid_ticker(self):
        ...
        try:
            ...
            ), f"Status code esperado 400, recebido {response.status_code}"
            data = response.json()
    ...
    def test_prediction_wrong_method(self):
        ...
        try:
            ...
            ), f"Status code esperado 405, recebido {response.status_code}"
    ...
    def test_endpoint_not_found(self):
        ...
        try:
            ...
            ), f"Status code esperado 404, recebido {response.status_code}"
