...
@click.option("--lookback", type=int, default=60, help="Lookback period for sequences")
@click.option("--hidden-size", type=int, default=50, help="LSTM hidden size")
@click.option("--num-layers", type=int, default=2, help="Number of LSTM layers")
...
@click.option(
    "--days-ahead", type=int, default=5, help="Days to predict (predict mode only)"
)
    ...
    try:
        ...
        if mode in ["predict", "both"]:
            ...
            predict_pipeline = PredictPipeline(
                model_path=model_path, ticker=ticker, lookback=lookback
            )
            ...
            predictions_df = predict_pipeline.predict(
                days_ahead=days_ahead, output_path=output_path
            )
