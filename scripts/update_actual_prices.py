...
def get_actual_price(ticker: str, date: str) -> Optional[float]:
    ...
    try:
        ...
        if len(future_dates) > 0:
            ...
            logger.info(
                f"{ticker}: Usando preço de {nearest_date.date()} para {date}: R$ {price:.2f}"
            )
            return float(price)
    ...
    for prediction in pending:
        ...
        if not dry_run:
            try:
                ...
                logger.info(
                    f"✅ Atualizado: {ticker} @ {pred_date} = R$ {actual_price:.2f}"
                )
                stats["success"] += 1
            except Exception as e:
            ...
            logger.info(
                f"[DRY RUN] Atualizaria: {ticker} @ {pred_date} = R$ {actual_price:.2f}"
            )
            stats["success"] += 1
...
def main():
    ...
    parser.add_argument(
        "--limit", type=int, help="Número máximo de predições a processar"
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
    ...
    for ticker in tickers:
        try:
            stats = update_predictions_for_ticker(
                firestore_svc, ticker, limit=args.limit, dry_run=args.dry_run
            )
