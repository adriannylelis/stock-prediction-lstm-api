...
def get_analytics(ticker: str):
    ...
    try:
        ...
        if not is_valid:
            return (
                jsonify(
                    {"success": False, "error": "Invalid ticker", "message": error_msg}
                ),
                400,
            )
        ...
        if not include_pending:
            predictions = [
                p for p in all_predictions if p.get("actual_price") is not None
            ]
        else:
            ...
            jsonify(
                {
                    ...
                    "data": {
                        ...
                        "predictions_with_actual": len(
                            [p for p in all_predictions if p.get("actual_price")]
                        ),
...
def get_pending_predictions(ticker: str):
    ...
    try:
        ...
        if not firestore_svc.is_available():
            return (
                jsonify({"success": False, "error": "Firestore service unavailable"}),
                503,
            )
        ...
        logger.error(
            f"Error in get_pending_predictions for {ticker}: {str(e)}", exc_info=True
        )
