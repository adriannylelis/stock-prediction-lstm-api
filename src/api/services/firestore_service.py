...
class FirestoreService:
    ...
    def save_prediction(self, data: dict) -> Optional[str]:
        ...
        try:
            ...
            logger.info(f"✅ Predição salva: {doc_id} | {data['ticker']} | {data['prediction_date']}")
            return doc_id
    ...
    def get_pending_predictions(self, ticker: Optional[str] = None) -> List[Dict]:
        ...
        """
        try:
            query = self.collection.where(filter=FieldFilter('actual_price', '==', None))
            ...
            logger.info(f"📋 {len(predictions)} predições pendentes" + (f" para {ticker}" if ticker else ""))
            return predictions
    ...
    def update_actual_price(self, ticker: str, prediction_date: str, actual_price: float) -> int:
        """
        ...
        try:
            ...
            if updated > 0:
                logger.info(f"✅ {updated} predições atualizadas para {ticker} em {prediction_date}")
            else:
                logger.debug(f"ℹ️ Nenhuma predição pendente encontrada para {ticker} em {prediction_date}")
    ...
    def get_accuracy_metrics(self, ticker: str, limit: int = 100) -> Dict:
        ...
        try:
            ...
            # Calcular métricas
            errors = [abs(p['actual_price'] - p['predicted_price']) for p in predictions]
            mae = sum(errors) / len(errors)
            
            mape = sum([
                abs((p['actual_price'] - p['predicted_price']) / p['actual_price']) * 100
                for p in predictions if p['actual_price'] != 0
            ]) / len(predictions)
