    ...
    def predict(self, ticker: str) -> Dict[str, Any]:
        try:
            ...
            if isinstance(df_features.columns, pd.MultiIndex):
                df_features.columns = [
                    col[0] if col[1] == "" else col[0] for col in df_features.columns
                ]
            ...
            if nan_counts.sum() > 0:
                ...
                print(
                    f"   Columns with NaN: {list(df_features.columns[nan_counts > 0])}"
                )
            ...
            if len(scaled_data) < lookback:
                raise InsufficientDataError(
                    ticker=ticker, required=lookback, available=len(scaled_data)
                )
            ...
            print(
                f"🔍 DEBUG Prediction after inverse_transform: {predicted_price_array}"
            )
            ...
            return {
                ...
                "change_direction": (
                    "alta"
                    if change_percent > 0
                    else "baixa" if change_percent < 0 else "neutra"
                ),
            ...
            if (
                "tensor" in error_msg
                or "shape" in error_msg
                or "dimension" in error_msg
            ):
                logger.error(
                    f"Erro de inferência do modelo para {ticker}: {str(e)}",
                    exc_info=True,
                )
                raise ModelInferenceError(ticker=ticker, error_detail=str(e))

            logger.error(
                f"Erro inesperado ao realizar previsão para {ticker}: {str(e)}",
                exc_info=True,
            )
            raise ModelInferenceError(ticker=ticker, error_detail=str(e))
