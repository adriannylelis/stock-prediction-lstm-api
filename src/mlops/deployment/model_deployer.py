    ...
    def _smoke_test(self, model_uri: str) -> tuple[bool, str]:
        ...
        try:
            ...
            if hasattr(model, 'ticker_embedding'):
                ...
                embedding_dim = model.embedding_dim if hasattr(model, 'embedding_dim') else 8
                lstm_input_size = model.lstm.input_size if hasattr(model, 'lstm') else 27
                input_size = lstm_input_size - embedding_dim
                logger.info(f"Embedding model: using {input_size} features (LSTM expects {lstm_input_size} = {input_size} + {embedding_dim})")
            else:
            ...
            with torch.no_grad():
                ...
                if hasattr(model, 'ticker_embedding'):
                    num_tickers = model.num_tickers if hasattr(model, 'num_tickers') else 8
